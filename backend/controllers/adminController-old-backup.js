// adminController.js - Firebase version
...
exports.createAdmin = async (req, res) => {
  try {
    ...
    const existingAdmin = await adminService.findByEmail({ where: { Email } });
    if (existingAdmin) {
      return res.status(400).json({
        success: false,
    ...
    let newAdmin;
    try {
      newAdmin = await adminserviceService.create({
        Name: Name.trim(),
...
exports.getAllAdmins = async (req, res) => {
  try {
    ...
    const admins = await adminserviceService.findAll({
      attributes: {
...
exports.updateAdmin = async (req, res) => {
  try {
    ...
    const updatedAdmin = await adminService.findById(id, {
...
exports.updateBooking = async (req, res) => {
  try {
    ...
    if (updateData.customerId) {
      const customer = await customerService.findById(updateData.customerId);
      if (!customer) {
        return res.status(404).json({ message: 'Customer not found' });
      }
    }
    ...
    if (updateData.serviceId) {
      const service = await serviceService.findById(updateData.serviceId);
      if (!service) {
        return res.status(404).json({ message: 'Service not found' });
      }
    }
...
exports.getRevenueReport = async (req, res) => {
  try {
    ...
    const revenue = await bookingService.findAll({
      where: {
        Status: { [Op.ne]: 'cancelled' }
      },
      attributes: [
